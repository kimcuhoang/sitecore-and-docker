...
Expand-Archive -Path (Join-Path $AssetInstallPath "\\*xp0xconnect.scwdp.zip") -DestinationPath $shardToolPath

Get-ChildItem -Path $shardToolPath | ForEach-Object{
...
Move-Item (Join-Path $dataPath ("\{0}`_log.ldf" -f $dbShardMapManager)) -Destination (Join-Path $DatabasesPath "\${dbShardMapManager}.ldf")
Move-Item (Join-Path $dataPath ("\{0}`_log.ldf" -f $dbShard0)) -Destination (Join-Path $DatabasesPath "\${dbShard0}.ldf")
Move-Item (Join-Path $dataPath ("\{0}`_log.ldf" -f $dbShard1)) -Destination (Join-Path $DatabasesPath "\${dbShard1}.ldf")

# Move databases to install directory
Move-Item (Join-Path $dataPath "\*Xdb.Collection*.mdf") -Destination $DatabasesPath
